  ...
  <div class="page-content">
    ...
    <el-row style="gap: 12px">
      <el-col :xs="24" :sm="12" :lg="6">
        <el-input placeholder="仓库分类名" v-model="searchForm.name"></el-input>
      </el-col>
    ...
    <art-table v-bind="tableData" selection @current-change="changePage" @size-change="changePageSizes">
      <template #default>
        <el-table-column label="仓库分类名" prop="name" min-width="120" />
    ...
    <!-- 表单 -->
    <el-dialog v-model="dialogVisible" :title="dialogType === 'add' ? '新增仓库分类' : '编辑仓库分类'" width="500">
      <el-form ref="formRef" :model="formData" :rules="rules" label-width="120px">
        <el-form-item label="仓库分类名" prop="name">
...
<script setup lang="ts">
  ...
  import type { WarehouseCategoryData } from '@/api'
  import { ElMessage, ElMessageBox } from 'element-plus'
  import { WarehouseCategoryService } from '@/api'
  import { PaginationData } from '@/types/axios'

  const tableData = ref<PaginationData<WarehouseCategoryData[]>>({
    records: [],
    total: 0,
    currentPage: 1,
    pageSize: 10
  ...
  async function getListData() {
    try {
      ...
      const res = await WarehouseCategoryService.getPage({ currentPage, pageSize, ...searchForm.value })
      if (res.success) {
  ...
  const rules = reactive<FormRules>({
    name: [
      { required: true, message: '请输入仓库分类名', trigger: 'blur' },
      { min: 2, max: 20, message: '长度在 2 到 20 个字符', trigger: 'blur' }
    ],
    desc: [{ required: true, message: '请输入仓库分类描述', trigger: 'blur' }],
      ...
      if (valid) {
        try {
          ...
          if (dialogType.value === 'add') {
            res = await WarehouseCategoryService.add(formData.value)
          } else {
            if (!rowId.value) {
              ElMessage.error('执行错误，用户ID不存在')
              return
            }
            res = await WarehouseCategoryService.update(rowId.value, formData.value)
          }
  ...
  async function del() {
    try {
      await ElMessageBox.confirm('确定删除该仓库分类吗？', '注销用户', {
        type: 'error',
      ...
      if (!rowId.value) {
        ...
        const res = await WarehouseCategoryService.delete(rowId.value)
        if (!res.success) {
          ElMessage.error(res.message)
        } else {
          getListData()
