    ...
    <div class="main">
        ...
        <div class="steps-content">
            ...
            <div v-if="current === 1" class="check">
                ...
                <a-divider orientation="right"> 金额总计： {{ form.price }} </a-divider>
            ...
            <div v-if="current === 2">
                <a-result status="success" title="Submitted Successfully" sub-title="Please wait for the administrator to review the delivery request.">
                    <template #extra>
                        <router-link to="/sale/record">
                            <a-button key="console" type="primary"> Go Back </a-button>
                        </router-link>
                        <a-button key="buy" @click="current = 0"> Submit Again </a-button>
                    </template>
                </a-result>
            </div>
        </div>
...
export default {
    ...
    mounted() {
        FindAllCommodity().then(res => {
            if (res.success) this.commodityList = res.data
        })
    },
