<template>
    <div>
        ...
        <a-table :loading="loading" :columns="columns" :data-source="data" bordered rowKey="id">
            <template v-for="col in ['name', 'gender', 'phone', 'id_card', 'department', 'address']" :slot="col" slot-scope="text, record, index">
                <div :key="col">
        ...
        <a-modal title="Title" :visible="visible" @ok="submitForm" @cancel="visible = false">
            <a-form-model :model="form">
                ...
                <a-form-model-item label="身份证号">
                    <a-input v-model="form.id_card" placeholder="请输入司机身份证信息" />
                </a-form-model-item>
    ...
    methods: {
        ...
        submitForm() {
            SaveEmployee(this.form).then(res => {
                if (res.success) this.$message.success('员工信息提交成功')
                this.visible = false
                this.loadTableData()
            })
        },
        ...
        save(id, index) {
            ...
            SaveEmployee(newData[index]).then(res => {
                if (res.success) this.$message.success('信息保存成功')
            })
        },
        cancel(id) {
            const newData = [...this.data]
        ...
        confirm(id) {
            DeleteEmployeeById(id).then(res => {
                if (res.success) this.$message.success('Delete success')
                this.loadTableData()
            })
        },
    },
