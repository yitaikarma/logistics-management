    ...
    <div>
        ...
        <div style="display: flex; margin-bottom: 20px">
            ...
            <a-form layout="inline" style="display: flex; align-items: center">
                <a-form-item>
                    <a-input v-model:value="formState.account" placeholder="账号"> </a-input>
                </a-form-item>
                <a-form-item>
                    <a-input v-model:value="formState.moudle" placeholder="模块名"> </a-input>
                </a-form-item>
                <a-button type="primary" html-type="submit" @click="handleSubmit">搜索</a-button>

                <!-- 导出操作日志 -->
                <download-excel class="export-excel-wrapper" style="margin-left: 20px" :data="data" :fields="json_fields" name="操作日志.xls">
    ...
    methods: {
        ...
        handleDelete(r, index) {
            DeleteSystemLogById(r.id).then(res => {
                if (res.success) this.$message.success('日志信息删除成功')
                this.loadTableData()
            })
        },
        //条件查询处理函数
