<template>
    <div>
        ...
        <a-table :loading="loading" :columns="columns" :data-source="data" bordered rowKey="id">
            <template v-for="col in ['name', 'gender', 'phone', 'score', 'idCard', 'address', 'license']" :slot="col" slot-scope="text, record, index">
                <div :key="col">
    ...
    methods: {
        ...
        submitForm() {
            SaveDriver(this.form).then(res => {
                if (res.success) this.$message.success('司机信息提交成功')
                this.visible = false
                this.loadTableData()
            })
        },
