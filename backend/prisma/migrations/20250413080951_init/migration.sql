...
-- CreateTable
CREATE TABLE `InventoryExtension` (
    `id` INTEGER UNSIGNED NOT NULL AUTO_INCREMENT,
    `total` INTEGER UNSIGNED NOT NULL DEFAULT 0,
    `desc` TEXT NULL,
    `status` TINYINT UNSIGNED NOT NULL DEFAULT 0,
    ...
    `inventoryId` INTEGER UNSIGNED NULL,

    INDEX `InventoryExtension_warehouseId_idx`(`warehouseId`),
    INDEX `InventoryExtension_inventoryId_idx`(`inventoryId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `Order` (
    `id` INTEGER UNSIGNED NOT NULL AUTO_INCREMENT,
    `fromProvince` VARCHAR(255) NULL,
    `fromCity` VARCHAR(255) NULL,
    `fromDistrict` VARCHAR(255) NULL,
...
-- AddForeignKey
ALTER TABLE `InventoryExtension` ADD CONSTRAINT `InventoryExtension_warehouseId_fkey` FOREIGN KEY (`warehouseId`) REFERENCES `Warehouse`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `InventoryExtension` ADD CONSTRAINT `InventoryExtension_inventoryId_fkey` FOREIGN KEY (`inventoryId`) REFERENCES `Inventory`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
