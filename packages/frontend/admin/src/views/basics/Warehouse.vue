<template>
    <div>
                ...
                <a-col :span="8" v-for="(item, index) in this.data" :key="index" class="item">
                    <a-card hoverable>
                        <template slot="actions" class="ant-card-actions">
                            ...
                            <span>
                                <router-link :to="'/warehouse/' + item.id"> <a-icon type="bar-chart" /> 库存管理 </router-link>
                            </span>
                        </template>
        ...
        <a-modal title="新增仓库" :visible="visible" @ok="submit" @cancel="visible = false">
            ...
            <a-input v-model="form.responsible" addon-before="仓库负责人" style="width: 300px"></a-input>
        </a-modal>
    </div>
</template>
...
export default {
    ...
    data() {
        return {
            visible: false,
            form: { id: '', responsible: '', name: '' },
            spinning: false,
            data: [],
            imgList: [require('../../assets/warehouse0.svg'), require('../../assets/warehouse1.svg'), require('../../assets/warehouse2.svg')],
        }
    },
