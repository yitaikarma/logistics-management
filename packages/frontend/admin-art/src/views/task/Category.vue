  ...
  <div class="page-content">
    ...
    <el-row style="gap: 12px">
      <el-col :xs="24" :sm="12" :lg="6">
        <el-input placeholder="任务分类名" v-model="searchForm.name"></el-input>
      </el-col>
    ...
    <art-table v-bind="tableData" selection @current-change="changePage" @size-change="changePageSizes">
      <template #default>
        <el-table-column label="任务分类名" prop="name" min-width="120" />
    ...
    <!-- 表单 -->
    <el-dialog v-model="dialogVisible" :title="dialogType === 'add' ? '新增任务分类' : '编辑任务分类'" width="500">
      <el-form ref="formRef" :model="formData" :rules="rules" label-width="120px">
        <el-form-item label="任务分类名" prop="name">
...
<script setup lang="ts">
  ...
  import type { TaskCategoryData } from '@/api'
  import { ElMessage, ElMessageBox } from 'element-plus'
  import { TaskCategoryService } from '@/api'
  import { PaginationData } from '@/types/axios'

  const tableData = ref<PaginationData<TaskCategoryData[]>>({
    records: [],
    total: 0,
    currentPage: 1,
    pageSize: 10
  ...
  async function getListData() {
    try {
      ...
      const res = await TaskCategoryService.getPage({ currentPage, pageSize, ...searchForm.value })
      if (res.success) {
  ...
  const rules = reactive<FormRules>({
    name: [
      { required: true, message: '请输入任务分类名', trigger: 'blur' },
      { min: 2, max: 20, message: '长度在 2 到 20 个字符', trigger: 'blur' }
    ],
    desc: [{ required: true, message: '请输入任务分类描述', trigger: 'blur' }],
      ...
      if (valid) {
        try {
          ...
          if (dialogType.value === 'add') {
            res = await TaskCategoryService.add(formData.value)
          } else {
            if (!rowId.value) {
              ElMessage.error('执行错误，用户ID不存在')
              return
            }
            res = await TaskCategoryService.update(rowId.value, formData.value)
          }
  ...
  async function del() {
    try {
      await ElMessageBox.confirm('确定删除该任务分类吗？', '注销用户', {
        type: 'error',
      ...
      if (!rowId.value) {
        ...
        const res = await TaskCategoryService.delete(rowId.value)
        if (!res.success) {
          ElMessage.error(res.message)
        } else {
          getListData()
