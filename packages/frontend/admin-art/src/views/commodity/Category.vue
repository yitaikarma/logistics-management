  ...
  <div class="page-content">
    ...
    <el-row style="gap: 12px">
      <el-col :xs="24" :sm="12" :lg="6">
        <el-input placeholder="商品分类名" v-model="searchForm.name"></el-input>
      </el-col>
    ...
    <art-table v-bind="tableData" selection @current-change="changePage" @size-change="changePageSizes">
      <template #default>
        <el-table-column label="商品分类名" prop="name" min-width="120" />
    ...
    <!-- 表单 -->
    <el-dialog v-model="dialogVisible" :title="dialogType === 'add' ? '新增商品分类' : '编辑商品分类'" width="500">
...
<script setup lang="ts">
  ...
  import type { CommodityCategoryData } from '@/api'
  import { ElMessage, ElMessageBox } from 'element-plus'
  import { CommodityCategoryService } from '@/api'
  import { PaginationData } from '@/types/axios'

  const tableData = ref<PaginationData<CommodityCategoryData[]>>({
    records: [],
    total: 0,
    currentPage: 1,
    pageSize: 10
  ...
  async function getListData() {
    try {
      ...
      const res = await CommodityCategoryService.getPage({ currentPage, pageSize, ...searchForm.value })
      if (res.success) {
  ...
  const rules = reactive<FormRules>({
    ...
    desc: [{ required: true, message: '请输入商品分类描述', trigger: 'blur' }],
      ...
      if (valid) {
        try {
          ...
          if (dialogType.value === 'add') {
            res = await CommodityCategoryService.add(formData.value)
          } else {
            if (!rowId.value) {
              ElMessage.error('执行错误，用户ID不存在')
              return
            }
            res = await CommodityCategoryService.update(rowId.value, formData.value)
          }
  ...
  async function del() {
    try {
      await ElMessageBox.confirm('确定删除该商品分类吗？', '注销用户', {
        type: 'error',
      ...
      if (!rowId.value) {
        ...
        const res = await CommodityCategoryService.delete(rowId.value)
        if (!res.success) {
          ElMessage.error(res.message)
        } else {
          getListData()
